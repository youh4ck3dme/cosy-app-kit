...
| “Isolated package / engine repo”           | Code lives under app tree `src/lib/builder` inside product checkout. Headless _in imports_, not _as a package boundary_.                          |
...
Non-determinism of ids/timestamps unless caller supplies them. Replay of _payloads_ can still be deterministic if ids are fixed.
