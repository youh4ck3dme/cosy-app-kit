...
/**
 * Scoped LLM edit via **Mistral only** (no local / heuristic AI).
...
export const applyScopedNodeEdit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ node: RawNode; source: "mistral" }> => {
    ...
    if (!mistralKey) {
      throw new Error(
        "Missing MISTRAL_API_KEY. Scoped edits require Mistral — no local AI. Add MISTRAL_API_KEY in server env / Lovable Cloud Secrets.",
      );
    }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Mistral scoped edit failed.";
      // Do not fall back to local/heuristic AI — surface the failure.
      if (message.includes("Missing MISTRAL_API_KEY")) throw error;
      throw new Error(
