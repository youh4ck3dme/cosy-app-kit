...
export default defineTool({
  ...
  inputSchema: {
    ...
    model: z.string().optional().describe("Mistral model id, e.g. mistral-large-latest. Optional."),
    system_prompt: z
      .string()
      .optional()
      .describe("Optional default system prompt for this thread."),
  },
  ...
  handler: async ({ title, model, system_prompt }, ctx) => {
    ...
    const row: Record<string, unknown> = {
      ...
      title,
      // Always set explicitly — never rely on legacy OpenAI/Gemini DB defaults.
